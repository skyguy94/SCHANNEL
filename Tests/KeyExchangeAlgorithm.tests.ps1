    ...
    Describe 'KeyExchangeAlgorithm is a DSC Resource' {
        ...
        Context "Meets DSC syntax requirements" {
            It 'Should be syntactically correct' {
                $res = Test-xDscSchema -Path "$PSScriptRoot\..\DSCResources\KeyExchangeAlgorithm\KeyExchangeAlgorithm.schema.mof"
                $res | Should Be $true
            }

            It 'Should be a well formed resource' {
                $res = Test-xDscResource -Name "$PSScriptRoot\..\DSCResources\KeyExchangeAlgorithm"
